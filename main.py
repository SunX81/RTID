...
def compress_channel(channel, base_fraction):
    ...
    # Create a mask to retain frequency components above the threshold
    mask = np.where(magnitude_spectrum > threshold, 1, 0)

    # Apply the mask to discard frequency components below the threshold
    dft = dft * mask

    # Perform inverse DFT transform
...
def process_image_with_mask(impath, ksize, gau_kz, sigma):
    ...
    # Create a resulting image and replace the blurred area with the original
